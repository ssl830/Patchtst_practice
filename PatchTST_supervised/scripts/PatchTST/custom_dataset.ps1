# Custom Dataset PatchTST Training Script for Windows PowerShell
# Usage: .\custom_dataset.ps1
...
# 预测长度循环 - 可根据需要修改预测窗口
# ===========================================
$pred_lengths = @(24, 48, 96, 192)

foreach ($pred_len in $pred_lengths) {
    Write-Host "Training PatchTST for prediction length: $pred_len" -ForegroundColor Green
    ...
    python -u run_longExp.py `
      ...
      --learning_rate $learning_rate | Out-File -FilePath $log_file
    
    Write-Host "Completed training for prediction length: $pred_len" -ForegroundColor Yellow
